...
function ResultCard({ icon: Icon, value, suffix, prefix, label, description, color, delay }: typeof results[0] & { prefix?: string }) {
  return (
    <FadeIn delay={delay} duration={0.7} direction="scale" margin="-60px" className="min-w-0 h-full">
      <div
        className="relative rounded-[2.5rem] p-8 overflow-hidden group cursor-pointer hover-lift h-full"
        style={{
...
const ResultsSection = () => (
  <section className="relative py-20 md:py-28 px-4 overflow-hidden" style={{ background: "#080312" }} id="results">
    ...
    <div
      className="absolute top-0 right-[-10%] w-[600px] h-[600px] rounded-full blur-[140px] opacity-20 pointer-events-none blob-pulse-1"
      style={{ background: "radial-gradient(circle, #f472b6, transparent 70%)" }}
    />
    <div
      className="absolute bottom-[-10%] left-[-10%] w-[800px] h-[800px] rounded-full blur-[160px] opacity-15 pointer-events-none blob-pulse-slow"
    ...
    <div className="container mx-auto relative z-10">
      ...
      <div className="flex flex-col lg:flex-row items-start justify-between gap-10 lg:gap-20 mb-20 md:mb-28">
        {/* Right Side: Titles */}
        <FadeIn duration={0.8} direction="left" margin="-100px" className="flex-1">
        ...
        {/* Left Side: Statement */}
        <FadeIn delay={0.2} duration={0.8} direction="right" margin="-100px" className="flex-1 lg:pt-16">
      ...
      {/* Bottom CTA */}
      <FadeIn delay={0.5} duration={0.7} direction="up" className="mt-24 text-center">
        ...
        <Link to="/form" onMouseEnter={prefetchForm}>
          <button
            type="button"
            className="cta-hover group relative inline-flex items-center justify-center gap-3 px-10 py-5 rounded-full text-lg font-black text-white shadow-[0_10px_40px_rgba(124,58,237,0.4)] overflow-hidden"
